...
function switchTab(tab) {
  ...
  const secoes = ['ativos', 'historico', 'configuracoes', 'caixa', 'lancar', 'whatsapp', 'mapa'];
  secoes.forEach(s => {
  ...
  else if (tab === 'mapa') carregarMapaMesas();
}
...
async function carregarMapaMesas() {
  ...
  try {
    ...
    const todasMesas = await resMesas.json();
    const pedidosAtivos = await resPedidos.json();

    grid.innerHTML = '';

    todasMesas.sort((a, b) => a.numero - b.numero).forEach(mesa => {
      const pedidoDaMesa = pedidosAtivos.find(p => p.mesa_id === mesa.id);
      ...
      if (pedidoDaMesa) {
        status = pedidoDaMesa.status === 'aguardando_fechamento' ? 'fechamento' : 'ocupada';

        // Calcula valor da mesa (subtotal + taxa)
        // Como o pedido.total pode não estar atualizado até o fechamento, recalculamos se necessário
        valorMesa = pedidoDaMesa.total || 0;

        if (pedidoDaMesa.created_at) {
          tempoMesa = calcularMinutos(pedidoDaMesa.created_at);
        }
      }

      const cardMesa = document.createElement('div');
      cardMesa.className = `mesa-item-mapa status-${status}`;
      cardMesa.onclick = () => {
        if (pedidoDaMesa) {
          switchTab('ativos');
          // Filtra pela mesa clicada
          const inputBusca = document.getElementById('filtro-mesas');
          if (inputBusca) {
            inputBusca.value = `Mesa ${mesa.numero}`;
            filtrarMesasAtivas(inputBusca.value);
          }
        } else {
          switchTab('lancar');
      ...
      cardMesa.innerHTML = `
        ${tempoMesa !== null ? `<span class="mesa-tempo">⏱️ ${tempoMesa}m</span>` : ''}
        <div class="mesa-num">${mesa.numero}</div>
        <div class="mesa-status-txt">${status === 'fechamento' ? '💰 CONTA' : status.toUpperCase()}</div>
        ${valorMesa !== null ? `<div class="mesa-valor">R$ ${valorMesa.toFixed(2)}</div>` : ''}
      `;

      grid.appendChild(cardMesa);
    });
  } catch (e) {
...
async function exibirPedidos() {
  ...
  const lists = {
    garcom: {
      recebidos: document.getElementById('list-garcom-recebidos'),
      servidos: document.getElementById('list-garcom-servidos'),
      pagamento: document.getElementById('list-garcom-pagamento')
    },
    balcao: {
      recebidos: document.getElementById('list-balcao-recebidos'),
      servidos: document.getElementById('list-balcao-servidos'),
      pagamento: document.getElementById('list-balcao-pagamento')
    }
  };

  if (!lists.garcom.recebidos || !lists.balcao.recebidos) return;

  isRenderingPedidos = true;

  // Limpa todas as colunas
  Object.values(lists).forEach(sub => Object.values(sub).forEach(el => el.innerHTML = ''));

  const counts = {
    garcom: { recebidos: 0, servidos: 0, pagamento: 0 },
    balcao: { recebidos: 0, servidos: 0, pagamento: 0 }
  };
  ...
  try {
    for (const pedido of pedidos) {
      ...
      let destino = 'servidos';
      if (isAguardando) destino = 'pagamento';
      else if (hasPend) destino = 'recebidos';

      const setor = pedido.mesa_id ? 'garcom' : 'balcao';
      counts[setor][destino]++;
      ...
      if (destino === 'recebidos' && pedido.created_at) {
        minutosCronometro = calcularMinutos(pedido.created_at);
        if (minutosCronometro >= 10) classeAlertaAtraso = 'alerta-borda-pisca';
      }

      card.className = `pedido-card status-${destino} ${pedido.id === pedidoAtualizadoId ? 'destaque-atualizacao' : ''} ${classeAlertaAtraso} ${isAguardando ? 'alerta-fechamento' : ''}`;
      ...
      const infoPagamento = (isAguardando && pedido.forma_pagamento) ? `
        <div style="background:#f9f9f9; padding:5px; border-radius:4px; margin-top:5px; font-size:0.85rem; border:1px solid #ddd;">
          ...
          ${(pedido.desconto > 0) ? `<br><span style="color:#e74c3c;"><strong>Desc:</strong> - R$ ${pedido.desconto.toFixed(2)}</span>` : ''}
        </div>` : '';

      card.innerHTML = `
        <div class="pedido-header" style="flex-direction: column; align-items: flex-start; gap: 5px;">
          <div style="display: flex; justify-content: space-between; width: 100%; align-items: center;">
            <h3 style="margin:0; font-size: 1.1rem;">${mesaNomeExibicao}</h3>
            <span style="font-weight: 900; color: #27ae60; font-size: 1rem;">R$ ${totalExibicao.toFixed(2)}</span>
          </div>
          <div style="display: flex; gap: 10px; font-size: 0.75rem; color: #7f8c8d;">
            <span>⏱️ ${minutosCronometro !== null ? `${minutosCronometro} min` : '--'}</span>
            <span>👤 ${pedido.garcom_id || 'N/I'}</span>
          </div>
        </div>
        
        <div class="pedido-itens" style="max-height: 150px; overflow-y: auto; margin: 8px 0; font-size: 0.85rem;">
          ${itens.map(item => `
            <div style="display:flex; justify-content:space-between; padding: 2px 0; border-bottom: 1px solid #f0f0f0; ${item.status === 'entregue' ? 'opacity: 0.5' : ''}">
              <span style="${item.status === 'entregue' ? 'text-decoration: line-through' : ''}">${item.quantidade}x ${item.nome}</span>
              <span style="font-weight:bold;">${item.status === 'entregue' ? '✓' : '⏳'}</span>
            </div>
          `).join('')}
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 5px; margin-top: 10px;">
          <button style="padding: 6px; font-size: 0.7rem; background: #34495e;" onclick='abrirModalEdicao(${JSON.stringify(pedido)}, ${JSON.stringify(itens)})'>✏️ EDITAR</button>
          ${isAguardando ? 
            `<button style="padding: 6px; font-size: 0.7rem; background: #27ae60;" onclick="aprovarFechamento(${pedido.id}, ${pedido.mesa_id})">💰 PAGAR</button>` :
            `<button style="padding: 6px; font-size: 0.7rem; background: #3498db;" onclick="abrirModalFechamentoAdmin(${JSON.stringify(pedido)})">💳 FECHAR</button>`
          }
        </div>
      `;

      lists[setor][destino].appendChild(card);
    }

    // Atualiza contadores globais das abas
    const countGarcom = counts.garcom.recebidos + counts.garcom.servidos + counts.garcom.pagamento;
    const countBalcao = counts.balcao.recebidos + counts.balcao.servidos + counts.balcao.pagamento;
    ...
    Object.keys(counts).forEach(setor => {
      Object.keys(counts[setor]).forEach(col => {
        const el = document.getElementById(`count-${setor}-${col}`);
        if (el) el.textContent = counts[setor][col];
      });
    });

    // Estados vazios
    ['garcom', 'balcao'].forEach(setor => {
      if (counts[setor].recebidos === 0 && counts[setor].servidos === 0 && counts[setor].pagamento === 0) {
